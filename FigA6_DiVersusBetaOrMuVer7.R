pdf("DiVersusBetaOrMuVer7.pdf", height = 4, width = 6)
...
yMin = -1.5;
plot(gamStor,log10((INought^(-gamStor))*DiStor),type="l",xlab="",ylab="",axes="FALSE",lwd=LineWidths[1],ylim=c(yMin,max(log10((INought^(-gamStor))*DiStor))));

#axis(2,pretty(range(log10((INought^(-gamStor))*DiStor)),5),pos=0);

yMax = max(log10((INought^(-gamStor))*DiStor));
axis(2,pretty(range(yMin:yMax),5),pos=0);
#axis(2,c(-1:5),pos=0);

if(R0r==1.5){
  xPos = 0.8*min(pretty(range(yMin:yMax),5));
  axis(1,pretty(range(gamStor),5),pos=xPos);
}else{
  xPos = 0.0; 
  #0.7*min(pretty(range(yMin:yMax),5));
  axis(1,pretty(range(gamStor),5),pos=xPos);
}
...
lgnd = paste(templgnd,R0r);
templgndB = "u=";
lgndB = paste(templgndB,u)
...
if(plotNum == 10) mtext(side=1,text=xaxisLbl,padj=2,cex=0.85);
if(plotNum == 5) mtext(side=2,text=yaxisLbl,padj=-1.5,cex=0.85);
if(plotNum==600){
xLocus = 0.75*(max(gamStor)-min(gamStor))
  yArg = log10((INought^(-gamStor))*DiStor);
  yLocus = 0.6*(max(yArg)-min(yArg))
  xLocus = 0.75*(max(gamStor)-min(gamStor))
  yLocus = 0.45*(max(yArg)-min(yArg))
  text(x=xLocus,y=yLocus,labels=lgndB,bty="n",cex=0.7);
  text(x=xLocus,y=yLocus,labels=lgnd,bty="n",cex=0.7);
} else{
  yArg = log10((INought^(-gamStor))*DiStor);
  xLocus = 0.85*(max(gamStor)-min(gamStor))
  yLocus = 0.55*(max(yArg)-min(yArg))
  text(x=xLocus,y=yLocus,labels=lgndB,bty="n",cex=0.7);
  yLocus = 0.35*(max(yArg)-min(yArg))
  text(x=xLocus,y=yLocus,labels=lgnd,bty="n",cex=0.7);
...
if(plotNum==6) legend(x=0.0,y=7.5,legend=lgnd,bty="n",lwd = c(3,2,1),cex=0.65,col=c("BLACK","BLUE","RED"));
